...
test.describe("public browsing", () => {
  test("opens catalog from home and reaches a resort detail page", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /зоны отдыха на алаколе/i })).toBeVisible();
    await page.getByRole("link", { name: /смотреть каталог/i }).click({ force: true });

    await expect(page).toHaveURL(/\/catalog$/);
    await expect(page.getByRole("heading", { name: /каталог зон отдыха/i })).toBeVisible();

    await page.getByRole("link", { name: /saffron coast family club/i }).first().click();

    await expect(page).toHaveURL(/\/catalog\/saffron-coast-family-club$/);
  ...
  test("submits a lead from the resort page and returns with success state", async ({ page }) => {
    ...
    await expect(page.locator("#lead-form").getByText(/заявка отправлена/i)).toBeVisible();
  });
